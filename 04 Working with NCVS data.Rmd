...
```{r comment="", results='hold'}
i <- sapply(data.pers12,levels)                        #gives factor levels for each variable 
...
{
   data.pers12[,xj] <- gsub("\\(([0-9]+)\\).*","\\1",data.pers12[,xj]) #remove the words that follow the                                                                         #numbers in parentheses
   data.pers12[,xj] <- as.numeric(data.pers12[,xj]) #convert the numbers in parentheses to just numeric                                                       #variables, which present as numbers w/o parentheses
}
```
