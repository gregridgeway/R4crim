...
* polygons - city boundaries, areas within 1000 ft of a school
...
```{r comment="", results='hold'} 
mapSZ <- st_read("10_shapefiles_and_data/allinjunctions.shp")
```
...
st_crs(mapSZ)
```
Of greatest importance is to notice that the projection is not latitude and longitude, although this is clearly the case from the previous plot, and that the unit of measurement is in feet. Whenever we compute a distance or area with these data, the units will be in feet or square feet.
...
```{r comment="", results='hold'}
mapCens <- st_read("10_shapefiles_and_data/tl_2014_06_tract.shp")
```
...
library(rjson)
# access ACS data from JSON
#   http://api.census.gov/data/2014/acs5/examples.html
# number of US households in 2015 and 2014
fromJSON(file="http://api.census.gov/data/2015/acs1/cprofile?get=NAME,CP02_2015_001E,CP02_2014_001E&for=us:*")
# track down variables using https://www.socialexplorer.com/data/ACS2014_5yr/metadata/
# total population in specific tracts
fromJSON(file="http://api.census.gov/data/2014/acs5?get=B00001_001E&for=tract:204920,205110&in=state:06+county:037")
# total population in all tracts in LA county
fromJSON(file="http://api.census.gov/data/2014/acs5?get=B00001_001E&for=tract:*&in=state:06+county:037")

# get total (B03002001), non-hispanic white (B03002003), non-hispanic black (B03002004), hispanic (B03002012)
dataRace <- fromJSON(file="http://api.census.gov/data/2014/acs5?get=B03002_001E,B03002_003E,B03002_004E,B03002_012E&for=tract:*&in=state:06+county:037")
# convert to data frame, use stringsAsFactors to leave text as character strings
...
# add race information to MS13 injunction area
plot(st_geometry(subset(mapCens,inMS13)))
plot(st_geometry(mapSZms13), border="red", lwd=3, add=TRUE)
...
# shade depending on percent hispanic
...
mapVSCSZ <- subset(mapSZ, gang_name=="Venice Shoreline Crips")

plot(st_geometry(st_buffer(mapVSCSZ, dist=5280)))
plot(st_geometry(mapVSCSZ), add=TRUE)

i <- st_intersects(mapCens, st_buffer(mapVSCSZ, dist=5280))
mapCens$nearVSC <- lengths(st_intersects(mapCens,
                                         st_buffer(mapVSCSZ, dist=5280))) > 0
plot(st_geometry(subset(mapCens, nearVSC)), border="salmon", add=TRUE)
...
labs <- with(mapCens, pop[nearVSC])
text(st_coordinates(st_centroid(subset(mapCens,nearVSC))), labels=labs, cex=0.5)
...
```{r comment="", results='hold'}
dataCrime <- st_as_sf(dataCrime, coords=c("lon","lat"))
```

We need to tell R that this spatial object has coordinates in latitude and longitude. We can either give the coordinate system using the longer form `"+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"` or try to remember that [EPSG 4269](# from http://www.spatialreference.org/ref/epsg/2229/) refers to latitude and longitude.
```{r comment="", results='hold'}
st_crs(dataCrime) <- "+init=epsg:4269"
```
...
```{r comment="", results='hold'}
i <- st_intersects(st_buffer(mapSZ[51,], dist=5280),
                   mapCens)[[1]]
plot(st_geometry(mapCens[i,]))
