...
a[-10:10 + i]
```
Note that the line for _The Genesis Code_ has the movie title in href, but no movie title is between the HTML tags `<a></a>`. In these cases let's pull the movie title from the href argument.
