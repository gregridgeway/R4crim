...
```{r echo=FALSE}
counterExercise <- 0
exNum <- function() {counterExercise <<- counterExercise+1; counterExercise}
```
...
`r exNum()`. Have R put in alphabetical order
...
# Exercises
`r exNum()`. What's the last state in the list?
...
# Exercises
`r exNum()`. Report `TRUE` or `FALSE` for each state depending on if you have lived there
`r exNum()`. With `a <- 1:100`, pick out odd numbers between 50 and 75
...
```
# Exercises
`r exNum()`. Use `sample()` to estimate the probability of rolling a 6
`r exNum()`. Use `sample()` to estimate the probability that the sum of two die equal 7
`r exNum()`. Use `sample()` to select randomly five states without replacement
`r exNum()`. Use `sample()` to select randomly 1000 states with replacement
...
# Exercises
`r exNum()`. Fix `state.list` so that "DC" is in "other" rather than "east". Here are a few hints.
     + access "other" using `$`
...
ls()
```
If you did not spell the name of the .RData file exactly correctly, then R will give you an error. A common occurence when downloading the same file from the web multiple times is for you web browser to add numbers to the multiple versions you've downloaded. So check the file name carefully. Here's what happens when I request a file that doesn't exist.
```{r comment="", results=TRUE, warning=TRUE, error=TRUE}
load("chicago crime.RData")
```

Let's check that this is indeed a dataset. You can use the `is()` command on any R object to ask it to identify itself.
```{r comment="", results='hold'}
is(chicagoCrime)
```
...
`r exNum()`. Count `NA`s in each column
`r exNum()`. Look up `Location.Description`, `Block`, `Beat`, and `Ward` for those missing `Latitude`
...
# Exercises
`r exNum()`. Use a for loop to create a variable `Coordinates` that looks like "(X.Coordinate,Y.Coordinate)"
...
```
# Exercises
`r exNum()`. How many assaults occurred in the street? (Location.Description=="STREET")
...
# Exercises
`r exNum()`. Make a barplot indicating how many states are in each region. Use state.list
`r exNum()`. Identify the beat with the most crimes
`r exNum()`. Identify the beat with the most domestic violence incidents
`r exNum()`. Part 1 crimes are homicide, robbery, assault, arson, burglary, theft, rape, 
