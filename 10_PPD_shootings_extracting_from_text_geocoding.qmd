...
```
There appear to be four tables on the page, but I noticed that the third one of these has an `id="data-table-ois"`. That must be the one we want. Let's extract it by name and convert it to an R data frame object.
```{r}
...
ois <- ois |>
   mutate(date = gsub(
".*?(January|February|March|April|May|June|July|August|September|October|November|December)( [0-9]{1,2})(, 20[0-9]{2})?.*", 
                      "\\1\\2\\3", text),
