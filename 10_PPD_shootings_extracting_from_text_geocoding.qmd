...
a <- scan("https://www.phillypolice.com/accountability/ois/", what="", sep="\n")
# search for the start of a table
i <- grep("<tbody>", a)
a[i] |> substring(1, 500)
# search for start of table <tbody> in a[i]
iStart <- gregexpr("<tbody>",  a[i]) |> unlist()
iEnd   <- gregexpr("</tbody>", a[i]) |> unlist()
a <- a[i] |> substring(iStart, iEnd)
a |> substring(1, 500)
```
...
```
Nothing will be visible on your screen at this point. If you want to watch the browser is doing in response to R actions, you can use `view()`, but this is not necessary.
```{r}
#| eval: false
b$view()
```
Let's tell our hidden browser to navigate over to the PPD OIS webpage. Sometimes pages take a moment to load. go_to() waits for the navigation to the page finish before completing.
```{r}
b$go_to("https://www.phillypolice.com/accountability/ois/")
```
I will grab a screenshot to show that everything is working so far.
```{r}
#| label: fig-screenshot
#| fig-cap: Screenshot from the hidden Chrome browser view of the PPD OIS webpage
# get page size
pageSize <- b$Page$getLayoutMetrics()$contentSize
b$screenshot(cliprect = c(left=0, top=0, 
                          width=pageSize$width, height=800))
knitr::include_graphics("screenshot.png")
```
Let's take a little cybersecurity detour for a moment. If you right-click on a webpage and select "Inspect," then you can see the DOM for the page. After inspection, I noticed that the OIS table's ID is `#data-table-ois`. I can tell the DOM to change elements on the page. To demonstrate, I will change the address in the 25th row to 3718 Locust Walk. `tr:nth-child(25)` selects the 25th row and `td:nth-child(2)` selects the column. Setting `.textContent = '3718 Locust Walk'` changes the cell text in the DOM immediately.
```{r}
b$Runtime$evaluate("document.querySelector('#data-table-ois tr:nth-child(25) td:nth-child(2)').textContent = '3718 Locust Walk';")
```
Now let's look at the page
```{r}
#| label: fig-screenshot2
#| fig-cap: Screenshot of the manipulated PPD OIS webpage
b$screenshot(cliprect = c(left=0, top=pageSize$height-800, 
                          width=pageSize$width, height=800))
knitr::include_graphics("screenshot.png")
```
This is how the "refund/overpayment scam" works. A fraudster gets the victim to view their account webpage and to give the fraudster some level of remote control. The fraudster then manipulates the DOM to give the impression that they have mistakenly sent the victim \$10,000 instead of \$1,000. The fraudster then convince the victim to send the \$9,000 difference even though no money was ever given to them.

Rather than try to scam someone, let's get back to the business of pulling all the OIS data into R. I will first get the ID of the main HTML from DOM. Then I will read in the HTML associated with that ID.
```{r}
# get the ID of the main HTML
html <- b$DOM$getDocument()$root$nodeId
html
# pull in raw html source code, like view page source or scan()
page_html <- b$DOM$getOuterHTML(nodeId = html)$outerHTML
...
```
There appear to be four tables on the page, but I noticed that the third one of these is called "data-table-ois." That must be the one we want. Let's extract it by name and convert it to R data frame object.
```{r}
...
page |> 
   ...
   html_attr("href")
```
With that, we have been able to get all of the information for the first 25 OIS incidents. Now we have to "click" the Next button to get to the next set of 25 OIS incidents. First, I will check to see if it is disabled. Since we are still looking at the first page it will not be disabled, but when we are looking at the final page of OIS incidents then it will be disabled. To figure out the name of that Next button, in my browser I right-clicked on the button, selected Inspect, and then reviewed the resulting HTML code.
```html
...
```
I see that the HTML tag is `<a>` and its classes are `paginate` and `next`. So `a.paginate.next` will search the HTML code for an `<a>` element with both a `paginate` and `next` class.
```{r}
# check whether the Next button is disabled
b$Runtime$evaluate(
  expression = 'document.querySelector("a.paginate_button.next").classList.contains("disabled");') |>
   pluck("result","value")
```
...
# check whether the Next button is disabled
b$Runtime$evaluate(
  expression = 'document.querySelector("#data-table-ois_next").classList.contains("disabled");') |>
   pluck("result","value")
```
...
b$Runtime$evaluate(
  expression = 'document.querySelector("#data-table-ois_next").click();')
```
...
while(!isFinished)
{
  ...
  # Pull the table rows
  #   page actually has 4 tables. 3rd one has the OIS data
  oisTable <- page |>
    html_elements("table#data-table-ois")
  ...
  oisURLs <- oisTable |>
    html_elements("tbody a") |> # get all <a> tags in the table body
    html_attr("href")
  ois[[iPage]]$url <- oisURLs

  # is "Next" button is disabled?
  isFinished <- b$Runtime$evaluate(
    expression = 'document.querySelector("#data-table-ois_next").classList.contains("disabled");') |>
     pluck("result","value")
...
```{r}
ois <- ois |>
   mutate(date = gsub(".*?(January|February|March|April|May|June|July|August|September|October|November|December)( [0-9]{1,2})(, 20[0-9]{2})?.*", 
                      "\\1\\2\\3", text),
          date = if_else(nchar(date)<20, date, NA),
          date = if_else(!is.na(date) & !grepl("20[0-9]{2}", date),
                         paste0(date, ", 20", substring(id,1,2)),
                         date),
          date = if_else(is.na(date),                         gsub(".*?([0-9]{1,2}[/-][0-9]{1,2}[/-](20)?[1-2][0-9])[^0-9].*",
                              "\\1", text),
                         date),
          date = mdy(date))
```
...
Our OIS data frame has the address for every incident, but to be more useful we really need the geographic coordinates. If we had the coordinates, then we could put them on a map, tabulate how many incident occur within an area, calculate distances, and answer questions about the geography of these data.
...
```
OIS 25-01 involves a dog shooting in Fairmount Park. The news stories about the incident place it 1 mile from Belmon/Edgely, close to Chamounix Drive and Ford Ave. 
```{r}
...
#| fig-cap: "Checking the location of OIS 24-14"
ois |>
   filter(id=="21-14") |>
   leaflet() |>
   addTiles() |>
...
`st_join()` will match each row in `ois` to each polygon in PSA. When linking a a data frame with _point_ data (OIS location) to a data frame with polygon data (like PSAs), the points will join with the polygons in which they land. I just want to add the `PSA_NUM` column out of the `PPDmap` to our `ois` data.
```{r}
PSAlookup <- ois |>
