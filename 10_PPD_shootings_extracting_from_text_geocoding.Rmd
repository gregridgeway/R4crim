...
library(ggmap)
gc.upenn <- geocodeOSM("3718 Locust Walk, Philadelphia, PA")
boundbox <- c(as.numeric(gc.upenn$lon)-0.004,
              as.numeric(gc.upenn$lat)-0.002,
              as.numeric(gc.upenn$lon)+0.004,
              as.numeric(gc.upenn$lat)+0.002)
...
ggmap(get_map(c(-75.0691987,40.0138755), zoom = 16)) +
   geom_path(aes(x=x, y=y),
             ...
             color="red",
             size= 2, alpha=0.5, arrow=arrow(length=unit(0.1,"inches")))
```
...
# Working with shapefiles and spatial joins

# load PPD shapefile
# review the directions at the top of the script for acquiring the shapefile
#    modify the path to the .shp file if needed here
#    like "Boundaries-PSA/Police_Service_Areas.shp"
PPDmap <- st_read("Boundaries_PSA/Boundaries_PSA.shp")
...
text(a[,1],a[,2],PPDmap$PSA_NUM,cex=0.7)

# explore the PPDmap object
#   PPDmap is an "simple features" type object
# tell us what "is" PPDmap
is(PPDmap)
# what's in PPDmap
PPDmap[1:3,]
# what is the "geometry" component?
is(PPDmap$geometry[1])
# what does the first polygon look like?
st_coordinates(PPDmap$geometry[1])
# how are these polygons stored?
a <- st_coordinates(PPDmap$geometry[1])
lines(a[,1],a[,2],col="red",lwd=3)
a <- st_coordinates(PPDmap$geometry[7])
lines(a[,1],a[,2],col="green",lwd=3)

# what is PPDmap's "coordinate reference system"?
st_crs(PPDmap)
# lcc=Lambert Conic Conformal
# NAD83=North American Datum
# WGS84=World Geodetic System
#    earth is not an ellipsoid but within some regions it is close

# Might need to transform to longlat so geocoded points can be overlaid
#   Projection parameters from proj4js
#   http://spatialreference.org/ref/epsg/

# Pennsylvania South projection
PPDmap <- st_transform(PPDmap, crs=st_crs("+init=epsg:2272"))
st_crs(PPDmap)
# Pseudo-Mercator projection (Google Maps, ESRI, OpenStreetMaps)
PPDmap <- st_transform(PPDmap, crs=st_crs("+init=epsg:3857"))
st_crs(PPDmap) # note units
# same, but now measures of distance will be in feet
PPDmap <- st_transform(PPDmap, crs=st_crs("+init=epsg:3857 +units=us-ft"))
st_crs(PPDmap) # note units
# back to standard latitude/longitude
...
points(lat~lon, data=gc.ois, col="red", pch=16)

# tabulate by police service area (PSA)
#   convert gc.ois to an sf object
a <- st_as_sf(gc.ois, coords=c("lon","lat"),
...
#    warnings about planar coordinates, EPSG 2272 gives more accurate results
PSAlookup <- st_join(gc.ois, PPDmap[,"PSA_NUM"])
PSAlookup[1:3,]

sort(table(PSAlookup$PSA))
i <- which(PSAlookup$PSA_NUM=="192")
plot(st_geometry(gc.ois[i,]), add=TRUE, col="green", pch=16)
i <- which(PSAlookup$PSA_NUM=="221")
plot(st_geometry(gc.ois[i,]), add=TRUE, col="blue", pch=16)

# shootings in the same district as UPenn
#  get UPenn coordinates
gc.upenn[,c("lon","lat")]
#  map them to the spatial coordinates
a <- st_as_sf(gc.upenn, coords=c("lon","lat"), crs=st_crs("+init=epsg:4326"))
st_join(a, PPDmap)
# now we see Penn is in district 183
i <- which(PSAlookup$PSA_NUM=="183")
# plot with Penn colors
plot(st_geometry(gc.ois[i,]), add=TRUE, col="red", bg="blue", pch=21)

# read more about incidents in PSA 183
a <- ois[PSAlookup$PSA_NUM=="183" & !is.na(PSAlookup$PSA_NUM),]
a



# color PPDmap by number of shootings
plot(st_geometry(PPDmap))
...
# break the shooting count into categories
a <- cut(PPDmap$nShoot,
...
# col <- rgb(seq(0,1,length=7),0,0,1)
# as.numeric will convert the factors in to integers 1-7
col[as.numeric(a)]

# create the map, color the regions by their n.shoot category
plot(st_geometry(PPDmap), col=col[as.numeric(a)], border=NA)
# add the number of shootings
a <- st_coordinates(st_centroid(PPDmap))
text(a[,1], a[,2], PPDmap$nShoot)


# save the final version
save(gc.ois, file="PPD OIS geocode.RData")

# to restart working with the data without having to rerun everything
library(lubridate)
setwd("Z:/Penn/CRIM602/data/PhillyOIS")
load("PPD OIS geocode.RData")
load("PPD OIS.RData")
