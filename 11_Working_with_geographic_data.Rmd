...
library(jsonlite)
```
Here's how you can access the ACS data on the total population of the United States in 2017. 
```{r comment="", results='hold', warning=FALSE}
fromJSON("https://api.census.gov/data/2017/acs/acs5?get=NAME,B01001_001E&for=us:*")
```
Let's deconstruct this URL. First, we're accessing data from the 2017 ACS data using `http://api.census.gov/data/2017/acs/`. Second, we're  using the date from the ACS sample collected over the last five years to estimate the total population... that's the `acs5` part. Third, we're accessing variable `B01001_001E`, which contains an estimate (the E at the end is for estimate) of the number of people in the United States. This we needed to track down, but the Social Explorer website, [https://www.socialexplorer.com/data/ACS2017_5yr/metadata/](https://www.socialexplorer.com/data/ACS2017_5yr/metadata/), makes this easier. Lastly, we asked `for=us:*`, meaning for the entire United States.

If we want the total number of people in specific census tracts, then we can make this request.
```{r comment="", results='hold', warning=FALSE}
fromJSON("https://api.census.gov/data/2017/acs/acs5?get=B01001_001E&for=tract:204920,205110&in=state:06+county:037")
```
...
```{r comment="", results='hold', warning=FALSE}
a <- fromJSON("https://api.census.gov/data/2017/acs/acs5?get=B01001_001E&for=tract:*&in=state:06+county:037")
# there are over 2000 census tracts in LA County. Show the first 10
a[1:10,]
```
You can find a lot more examples at [https://api.census.gov/data/2017/acs/acs5/examples.html](https://api.census.gov/data/2017/acs/acs5/examples.html).
...
```{r comment="", results='hold', warning=FALSE}
dataRace <- fromJSON("https://api.census.gov/data/2017/acs/acs5?get=B03002_001E,B03002_003E,B03002_004E,B03002_012E&for=tract:*&in=state:06+county:037")
```
We will convert the matrix to a data frame, leaving out the first row of the matrix that has the column names and telling R not to convert the character strings to factors.
```{r comment="", results='hold'}
...
dataRes <- fromJSON("https://api.census.gov/data/2017/acs/acs5?get=B01001_001E&for=tract:*&in=state:06+county:037")
