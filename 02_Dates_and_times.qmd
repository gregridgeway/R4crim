...
# Working with dates
Once `lubridate` is installed, you need to load it once in each R session where you wish to use its features. 
```{r}
#| comment: ""
#| message: false
library(lubridate)
...
b
```
`b` now stores those five dates in a format that recognizes the month, day, and year. `is(b)` tells us that R is storing `b` as a date object. There are different functions for other date formats depending on the ordering of the day, month and year, like `dmy()` and `ymd()` and even `mdy_hms()` for month, day, year, hours, minutes, seconds format.
