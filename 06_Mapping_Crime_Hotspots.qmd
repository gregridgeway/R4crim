...
file.info("Crimes_-_2001_to_Present.csv") |>
   mutate(size_gb = size / 10^9, .keep = "none") # .keep drops other columns
```
...
file.info("Crimes_-_2001_to_Present.csv.gz") |>
   mutate(size_gb = size / 10^9, .keep = "none")
```
...
```
If we see any of these dates with a time between 2:00 AM and 2:59 AM, we will add one hour to them... Some officer did not set their watch ahead, but we can do that for them.
```{r}
crime <- crime |>
...
It is a good time to save our cleaned up data to a file so we can load it quickly in the future without having to repeat all of the cleanup steps.
```{r}
save(crime, iucrLookup, file = "dataChicagoCrime.RData", compress=TRUE)
```
Whenever you need to work with this data again, you can load it quickly with `load("dataChicagoCrime.RData")`. This will recreate the `crime` and `iucrLookup` objects in your R session.
...
#| fig-cap: "Clustered map of reported assaults in Chicago, 2026 to date"
crime  |>
   filter(valid_coordinates &
             year(date) == 2026) |>
...
#| fig-cap: "Unclustered motor vehicle theft point map, Chicago, 2024"
vehicle_theft <- crime |>
...
#| fig-cap: "Clustered motor vehicle theft point map, Chicago, 2024"
vehicle_theft_map |>
   addCircleMarkers(lng = ~longitude,
                    lat = ~latitude,
                    radius = 2,
