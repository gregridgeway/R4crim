...
```
In R, `NA` means a missing value. We will do more exercises later using data containing some `NA` values. In any dataset in the wild, you are virtually guaranteed to find some `NA`s. The function `is.na()` helps determine whether there are any missing values (any NAs). In some of the problems below, we will use `is.na()`.

One strange but important thing about `NA` is that R treats it as "unknown." If you ask whether one unknown value equals another unknown value, R still cannot know the answer. This surprises a lot of new R users.
```{r}
NA == NA
is.na(NA)
```
Use `is.na()` when you want to test whether a value is missing. Do not use `== NA`.
...
# Things R will complain about

When you are learning R, you will see error messages. This is normal. Error messages are R's way of saying, "I cannot do what you asked me to do yet."

R is case sensitive. That means `Attendees`, `attendees`, and `ATTENDEES` are three different names. If you ask R for an object that does not exist, it will complain that the object is not found.
```{r}
#| error: true
attendees
```

R also needs all function inputs to be separated by commas. If a comma is missing, R often says something like "unexpected numeric constant" or "unexpected symbol."
```{r}
#| error: true
c(1 2 3)
```

R needs opening and closing parentheses to match. If you start a function with `(`, you need to close it with `)`. If you neglect the closing parenthesis, like `sqrt(25`, R cannot finish reading the code. In the console, it will usually display a `+`, R's way of saying "I'm waiting for you to finish your thought. You're leaving me hanging here." When rendering this document, R reports it as an incomplete expression.
```
> sqrt(25
+ 
```
To get out of this, either type the missing `)` or hit escape to abandon the calculation.

Text values need quotes. Without quotes, R thinks you are naming an object.
```{r}
#| error: true
c(yes, no, yes)
```

File names and folder names have to be exactly right. If R says that it cannot open a file or that a file does not exist, check the spelling, the folder, and the working directory. (We will load this file later, but here it is missing an "s" in `protests.RData`)
```{r}
#| error: true
load("protest.RData")
```

The fastest debugging strategy is usually to read the error message, check spelling and capitalization, check commas and parentheses, and then run just one line at a time.

# Setting the working directory
Now that we have covered a lot of fundamental R features, it is time to load in a real dataset. However, before we do that, R needs to know where to find the data file. So we first need to talk about "the working directory". When you start R, it has a default folder or directory on your computer where it will retrieve or save any files. You can run `getwd()` to get the current working directory. Here's my current working directory, which will not be the same as yours.
```{r}
getwd()    
```
...
Use `setwd()` to tell R what folder you want it to use as the working directory. If you do not set the working directory, R will not know where to find the data you wish to import and will save your results in a location in which you would probably never look. For this example, we are going to use `setwd()` as the first line of this script. Later we will talk about setting up a more portable R Studio project.
```{r}
#| eval: false
...
Now you can use R functions to load in any datasets that are in your working folder. If you have done your `setwd()` correctly, you should not get any errors because R will know exactly where to look for the data files. If the working directory that you have given in the `setwd()` is not right, R will think the file does not even exist. For example, if you give the path for, say, your R4econ folder, R will not be able to load data because the file is not stored in what R thinks is your working directory. With that out of the way, let's load a dataset.
...
slice(dataProtest, 1)
```
There is a more modern "grammar" in R using the pipe operator. This is a way to chain together functions in a more readable way. The pipe operator is `|>`. It takes the output of the function on the left and passes it as the first argument to the function on the right. Here is the same code as above using the pipe operator.
```{r}
dataProtest |> slice(1)
```
This code takes `dataProtest` and passes it in to the first argument of the `slice()` function. The `slice()` function then returns the first row of the dataset. The code is more readable this way. 
...
The square brackets `[]` are used to pick out parts of an object by position. This is called indexing. R starts counting at 1 (I note this because some programming languages start at 0).
```{r}
dataProtest$Date[1]
dataProtest$Date[1:5]
```

For a vector, like `dataProtest$Date`, we use one position inside the square brackets. For a data frame, which has rows and columns, we use two positions: `[row, column]`. The row position comes first, then a comma, then the column position.
```{r}
dataProtest[1, 1]
dataProtest[1, ]
dataProtest[, 1] |> head(10)
```

The code `dataProtest[1, 1]` returns the value in the first row and first column. The code `dataProtest[1, ]` returns the entire first row. The code `dataProtest[, 1]` returns the entire first column. Leaving one side of the comma blank means "give me all of them."

You can also use column names inside the square brackets.
```{r}
dataProtest[1:5, c("Date", "Location")]
```

In this script we will mostly use `slice()`, `select()`, `filter()`, and the `$` operator because they are often easier to read. But you will see bracket indexing in a lot of R code, so it is worth knowing the basic pattern.
...
When naming objects in R, choose names that are short, clear, and meaningful. Object names can contain letters, numbers, underscores (`_`), and periods (`.`), but they should not start with a number and should not contain spaces. As previously noted, R is case sensitive, so `protestData`, `ProtestData`, and `protestdata` are three different objects. I like starting with a lowercase label of what the object is, like the `data` in `dataProtest`. I tend to use "camel case," which capitalizes separate words in an object name, like `listIceCreamFlavors`. Clear names make your code easier to read, easier to debug, and easier to return to later when you no longer remember exactly what you were thinking.
...
dataProtest <- dataProtest |>
   ...
   mutate(state = toupper(state),
          state = replace_values(state,
                                 "CE" ~ "Space",
                                 "TE" ~ "IN"))
dataProtest |> 
   count(state)
```
Several things are happening here. First, the `toupper()` function is used to make sure that all state abbreviations are in upper case. Then we are using `replace_values()` to change the state abbreviations that need special fixes. Note its structure. The first argument is the variable that we are changing (`state`). Then we list all the replacements that we want to make. We are changing "CE" to "Space" and "TE" to "IN". Finally we rerun the `count()` function to see if our changes worked. All looks good now.
...
```
Now we can see the protester counts for civil rights and non-civil rights protest for each state. However, it is not clear how to compare the rows within each state. 
...
dataProtest |>
   ...
   pivot_wider(names_from = civilrights,
               ...
               names_prefix = "CR")
```
Almost done. We just need to filter those with fewer civil rights protesters.
```{r}
dataProtest |>
   group_by(state, civilrights) |>
   summarize(totAttendees = sum(Attendees, na.rm=TRUE)) |>
...
dataProtest |>
  ...
  filter(CR1 < CR0)
```
This approach requires a little trick with the `if_else()`, including the value of `Attendees` in the sum if the associated protest is/is not a civil rights protest. Still, having `pivot_wider()` in your toolbox will come in handy.
...
dataProtest |>
   ...
   head()
```
Second, we can get the total number of protesters by state and time period.
```{r}
dataProtest |>
   filter(Date < "2021-01-01") |>
   mutate(year = substring(Date, 1, 4) |> as.numeric(),
...
dataProtest |>
   ...
   arrange(state)
```
Finally, we are in a position to compute the percent change, sort, and display those states with the largest percentage change.
```{r}
dataProtest |>
   filter(Date < "2021-01-01") |>
   mutate(year = substring(Date, 1, 4) |> as.numeric(),
...
dataProtest |>
  ...
  print(n=Inf)
```
Why is it complaining about Guam? `separate_longer_delim()` can help here.
```{r}
dataProtest |>
  filter(state == "GU") |>
  separate_longer_delim(Tags, delim = "; ") |> # from tidyr
  count(Tags) |>
  arrange(desc(n))
```
Civil Rights and Other are themes in six protests... a tie.
...
* `[]` index an object to pick out selected values, rows, or columns
* `nrow()`, `ncol()`
* `head()`, `tail()`
...
* `replace_values()` used inside `mutate()` to change selected values in a column 
* `group_by()`, `summarize()`, `reframe()` used to summarize data by groups
* `arrange()` sort rows in a dataset
* `pivot_wider()` and `pivot_longer()` to organize data in wide or long form
