...
```{r comment="", results='hold'}
library(lubridate)
dates.list <- data.frame(date=seq(ymd("2010-01-01"),ymd("2016-10-01"),by="days")) #create a sequence of all days from Jan 1, 2010 to Oct 1, 2016, put in dataframe
dates.list$years  <- year(dates.list$date) #add a column 'year' to dataframe, consisting of the year of each date
dates.list$months <- sprintf("%02d",month(dates.list$date)) #add a column 'month' to dataframe, consisting of the month of each date
dates.list$days   <- sprintf("%02d",day(dates.list$date)) #add a column 'day' to dataframe, consisting of the day (of the month) of each date 
```

Let's look at the first couple of rows of `dates.list`:
```{r comment="", results='hold'}
head(dates.list)
```

Now, we have a dataframe (`dates.list`) where each row represents one date. The first column is the complete date, and subsequent columns represent a particular element of the date i.e. year, month, day. But for each day `dates.list`, we have information on multiple movies showing that day, and the gross for each movie. So a single dataframe will not be sufficient to store all that information. We're going to use a vector of data frames to collect the results. As you may or may not recall, a vector is defined as a sequence of data elements of the same type. The elements of this vector will be dataframes. 

This vector will be `results`, which we create by using the `vector` command. `results` will be empty at first: 
...
```{r comment="", results='hold'}
library(mailR)
send.mail(from = "gregridgeway@gmail.com", #replace with your email address
